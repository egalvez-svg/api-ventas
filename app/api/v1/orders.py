...
table_router = APIRouter(prefix="/branches/{branch_id}/tables", tags=["orders"])
...
async def get_order_invoice(
    ...
    await order_service.get(session, branch_id, order_id, user)
    return await invoice_service.generate_pre_boleta(session, order_id)


@table_router.get("/{table_id}/invoice", response_model=dict)
async def get_table_invoice(
    branch_id: int, table_id: int, session: SessionDep, _user: CurrentUser
):
    from app.services.invoice_service import invoice_service
